...
def generate_animation(
        video, save_path, prompt, negative_prompt, width, height):
    editor = MMEdit(model_name='controlnet_animation')

    editor.infer(video=video,
                 prompt=prompt,
...
def generate_background(config, width, height):
    ...
    back_ground_image = StableDiffuser.infer(config.bg_prompt,
                                             width=width,
                                             height=height,
                                             seed=config.bg_seed)
...
def replace_background_with_sam(config, back_ground_image):
    ...
    frame_files = [os.path.join(config.middle_video_frame_path, f)
    ...
    for ind in tqdm(range(len(all_images))):
        ...
        masks, _, _ = predictor.predict(point_coords=point_coord,
...
if __name__ == '__main__':
    ...
    generate_animation(video=config.source_video_frame_path,
    ...
    back_ground_image = generate_background(config,
                                            width=config.width,
                                            height=config.height)
