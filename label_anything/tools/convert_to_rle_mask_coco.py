...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Label studio convert to Coco fomat')
    parser.add_argument(
        '--json_file_path',
        default='project.json',
        help='label studio output json')
    parser.add_argument(
        '--out_dir',
        default='coco_format_files',
        help='output dir of Coco format json')
    parser.add_argument(
        '--classes',
        default=None,
        help='Classes list of the dataset, if None please check the output.')
...
def format_to_coco(args):
    ...
    if sys.platform == 'linux':
        image_path_from = os.path.join(
            '~/.local/share/label-studio/media/upload/',
            os.path.dirname(contents[0]['data']['image']).split('/')[-1])
    elif sys.platform == 'win32':
        image_path_from = os.path.join(
            '~/AppData/Local/label-studio/label-studio/media/upload/',
            os.path.dirname(contents[0]['data']['image']).split('/')[-1])
    else:
    ...
    for index_annotation in tqdm(range(len(contents))):
        ...
        if result == False:
            width_from_json = contents[index_annotation]['annotations'][0][
                'result'][0]['original_width']
            height_from_json = contents[index_annotation]['annotations'][0][
            ...
            for i in range(len(labels)):
                ...
                image_json_name_ = contents[index_annotation]['data'][
                    'image'].split('/')[-1]
                ...
                if not any(d['name'] == category
                           for d in coco_format['categories']):
                    coco_format['categories'].append({
                        'id':
                        len(coco_format['categories']),
                        'name':
