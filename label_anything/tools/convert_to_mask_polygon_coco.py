...
import jinja2
from ../sam/filter_poly import NearNeighborRemover
import shutil
...
def format_to_coco(args):
    ...
    if sys.platform == 'linux':
        ...
        print("JSON Contents:", contents[0])
        print("JSON Contents:", contents[0]['data']['image'])
        print("JSON Contents:", contents[0]['data']['image'].split('/')[-1])
    ...
    for index_annotation in tqdm(range(len(contents))):
        ...
        if result == False:
            ...
            for i in range(len(labels)):
                ...
                mask=rle2mask(rle,height_from_json,width_from_json) #由rle转换为mask
                contours, hierarchy = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #处理mask获取polygon
                point_list = []
                ...
                filter_points=NearNeighborRemover(distance_threshold=0.4).remove_near_neighbors(point_list)
                polygon_points=[]
                polygon_points[0]=[]
                for point in filter_points: #coco数据集polygon格式
                    polygon_points[0].append(point)
                new_contours = []
                for contour in contours:
