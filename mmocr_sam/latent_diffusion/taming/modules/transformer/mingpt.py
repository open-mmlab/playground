"""taken from: https://github.com/karpathy/minGPT/ GPT model:
...
class CausalSelfAttention(nn.Module):
    ...
    an explicit implementation here to show that there is nothing too scary
    here.
    """
...
class GPT(nn.Module):
    ...
    def forward_with_past(self,
        ...
        if past is not None:
            ...
            past = torch.cat(
                past, dim=-2)  # n_layer, 2, b, nh, len_past, dim_head
...
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None):
    ...
    Clearly the sampling has quadratic complexity unlike an RNN that is only
    linear, and has a finite context window of block_size, unlike an RNN that
    has an infinite context window.
    """
